...
#define WIFI_SSID "UltraNova24N"
#define WIFI_PASS "9bright7"
...
void send_duration( char* buf, u16_t start, u16_t end ) {
	...
	printf("Row has %i values, allocated %i bytes [String length: %i]\n", count, size, end-start);
...
static void http_server_netconn_serve(struct netconn *conn) {
	char* S_CONTENTLENGTH   = "Content-Length: ";
	int N_CONTENTLENGTH = 16;//1234567890123456
	struct netbuf *inbuf;
	char *buf, *buf1, *buf2, *sContent;
	u16_t buflen, buflen1, buflen2, buftotal;
	u16_t rmt_start;
	err_t err = ERR_OK;
	...
	while(netconn_recv( conn, &inbuf) != ERR_OK) {
		vTaskDelay(1000 / portTICK_PERIOD_MS ); // vTaskDelay( webSHORT_DELAY );
	}
	...
	if (err == ERR_OK) {
		...
		buf = NULL;
		do {
			do { // loop through loops
				/* obtain a pointer to the data in the fragment */
				netbuf_data(inbuf, (void**)&buf1, &buflen1);
				buflen2 = buflen + buflen1;
				buf2 = (char *)malloc(buflen2+6);
				if(buflen==0) { // If it is beggining of the read of the request, we search for content length value
					strncpy(buf2, buf1, buflen1);
					sContent = strstr(buf2, S_CONTENTLENGTH);
					if(sContent!=NULL) {
						buftotal = strtol(sContent+N_CONTENTLENGTH, NULL, 10) + find_body(buf2, buflen2); //atoi(&buf2[pstart+16]) + find_body(buf2, buflen2);
						printf("MSG Size: %i\n", buftotal); 
					} 
				}
				printf("Fragment size: %i (total read: %i from %i)\n", buflen1, buflen2, buftotal);
				strncpy(buf2, buf, buflen); // copy original
				strncpy(&buf2[buflen], buf1, buflen1); // append fragment
				if (buflen>0) { // if this isn't first fragment free current buffer to assign new one
					free(buf);
				}
				buf = buf2;
				buflen = buflen2;
			} while((netbuf_next(inbuf) >= 0) && (buftotal!=0));			
			if((buflen<buftotal) && (buftotal!=0)) { 
				netbuf_delete(inbuf);
				netconn_recv( conn, &inbuf); 
			};
		} while(buflen<buftotal);
		
		printf("Done reading request packets.\n");
		// Is this an HTTP POST command?
		if ( buflen>5 && 
			buf[0]=='P' && buf[1]=='O' && buf[2]=='S' && buf[3]=='T' ) {
			
			printf("Decoding POST request - Length: %i\n", buflen);
